        ...
        try:
            if args.source_repo is not None:
                lifecycle_hooks.run_source_repo_hook(
                    args=args, github_app_wrapper=github_app_wrapper, temp_script_dir=temp_script_dir
                )
        except Exception as e:
            logging.error(f"Error running source repo hook: {str(e)}", exc_info=True)  # Log the full stack trace
            sys.exit(1)

        try:
            hooks = lifecycle_hooks.LifecycleHooks(tmp_script_dir=temp_script_dir, args=args)
        except Exception as e:
            logging.error(f"Error occurred while initializing lifecycle hooks: {str(e)}", exc_info=True)
            sys.exit(1)
