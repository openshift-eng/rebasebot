...
class TestCliArgParser:
    ...
    def test_always_run_hooks_flag_parsing(self, get_valid_cli_args):
        # Test flag is True when provided
        args = get_valid_cli_args({"always-run-hooks": None})
        with patch("sys.argv", ["rebasebot", *args]):
            parsed_args = _parse_cli_arguments()
        assert parsed_args.always_run_hooks is True

        # Test flag is False when not provided (create minimal valid args without the flag)
        minimal_args = {
            "source": "https://github.com/kubernetes/autoscaler:master",
            "dest": "openshift/kubernetes-autoscaler:master",
            "rebase": "rebasebot/kubernetes-autoscaler:rebase-bot-master",
        ...
        args = args_dict_to_list(minimal_args)
        with patch("sys.argv", ["rebasebot", *args]):
            parsed_args = _parse_cli_arguments()
        assert parsed_args.always_run_hooks is False

    @patch("rebasebot.bot.run")
    def test_always_run_hooks_flag_passed_to_bot(self, mocked_run, get_valid_cli_args, tempfile):
        args = get_valid_cli_args({
            "github-user-token": tempfile,
            "always-run-hooks": None
        })
        ...
        assert "always_run_hooks" in call_kwargs
        assert call_kwargs["always_run_hooks"] is True
