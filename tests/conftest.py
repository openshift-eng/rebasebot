...
class CommitBuilder:
    ...
    def commit(self, commit_msg: str, committer_email=None) -> Commit:
...
def init_test_repositories() -> YieldFixture[Tuple[GitHubBranch, GitHubBranch, GitHubBranch]]:
    ...
    CommitBuilder(dest_gh_branch).add_file("another_file.go", _ANOTHER_GO_CODE).commit("UPSTREAM: <carry>: our cool addition")
